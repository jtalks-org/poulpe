...
/**
 * Currently Anonymous users can get access to VIEW_TOPICS branch permission only. The idea of the Test is to test the
 * order of getting Security groups list for a given Branch Permission. In the case of "Anonymous supported" Branch
 * Permission there should be only one method called - SecurityGroupList.getAllGroups(), whereas for the "Anonymous not
 * supported" Branch Permission Anonymous groups shall be removed from the list by calling
...
public class SecurityGroupListForBranchPermissionTest {
    private SecurityGroupListForBranchPermission securityGroupListForBranchPermission;
    ...
    public Object[][] getBranchPermissionsForNotAnonymous() {
        ...
        for (Object[] branchPermissionForAnonymous : getBranchPermissionsForAnonymous()) {
            branchPermissionList.remove(branchPermissionForAnonymous[0]);
        }
