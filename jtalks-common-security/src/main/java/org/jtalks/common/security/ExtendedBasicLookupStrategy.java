...
import org.jtalks.common.security.acl.SidFactory;
 ...
 * database and standard ANSI SQL features. If you are willing to sacrifice either of these constraints (e.g. use a
 * particular database feature such as hierarchical queries or materalized views, or reduce normalisation) you are
 * likely to achieve better performance. In such situations you will need to provide your own custom
 ...
 * <tt>lookupObjectIdentities</tt>. These are built from the same select and "order by" clause, using a different where
 * clause in each case. In order to use custom schema or column names, each of these SQL clauses can be customized, but
 * they must be consistent with each other and with the expected result set generated by the the default values.
 *
 * @author Ben Alex
 */
public final class ExtendedBasicLookupStrategy implements LookupStrategy {
     ...
     */
    public ExtendedBasicLookupStrategy(DataSource dataSource, AclCache aclCache,
     ...
     * contain all SIDs. If you have serious performance needs (e.g. a very large number of SIDs per object identity),
     * you'll probably want to develop a custom {@link LookupStrategy} implementation instead.
     * <p/>
     ...
     *         the located {@link Acl} (never <tt>null</tt> although some entries may be missing; this method should not
     *         throw {@link NotFoundException}, as a chain of {@link LookupStrategy}s may be used to automatically
     *         create entries if required)
     */
    ...
    /**
     * Sets the {@code PermissionFactory} instance which will be used to convert loaded permission data values to {@code
     * Permission}s. A {@code DefaultPermissionFactory} will be used by default.
     *
    ...
    private class ProcessResultSet implements ResultSetExtractor<Set<Long>> {
        ...
        /**
         * Accepts the current <code>ResultSet</code> row, and converts it into an <code>AclImpl</code> that contains a
         * <code>StubAclParent</code>
         *
        ...
        private void convertCurrentResultIntoObject(Map<Serializable, Acl> acls, ResultSet rs) throws SQLException {
            ...
            if (acl == null) {
                ...
                Sid owner = SidFactory.create(rs.getString("acl_sid"), rs.getBoolean("acl_principal"));
